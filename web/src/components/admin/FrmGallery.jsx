...
import {uploadPhoto, parseImages} from "../../services/photoService";
...
class FrmGallery extends Form {
  ...
  schema = Joi.object({
    ...
    format: Joi.string()
      ...
      .regex(
        ...
        "Solo estan permitidos ficheros del tipo imagen"
      )
      .label("Formato"),
  });
  ...
  processUpload = async () => {
    try {
      const { status, data } = await uploadPhoto(this.state.data);

      if (status === 200) {
        const galleryList = [...this.context.gallery.galleryList];
        const parsedImage = parseImages(Array.from(data));
        galleryList.push(parsedImage[0]);
        this.context.gallery.setGalleryList(galleryList);
            ...
            <div style={styles.imageAreaContainer}>
              {!croppedImage && (
                <div className="preview" style={styles.imagePlaceholder}>
                  ...
                  >
                    Selecione una imagen para previsualizarla
                  </p>
                </div>
              )}
              {croppedImage && (
...
FrmGallery.contextType = AppContext;
