...
router.post('/', async (req, res) => {
    ...
    const schema = Joi.object({
        image: Joi.string().base64().required().custom((value, helpers) => {
            if (new Buffer.from(value, 'base64').length > config.get('upload.maxSize')) //1048576 bytes = 1MB
                return helpers.message(`Image should not be greater than ${config.get('upload.maxSize')} ${config.get('upload.unit')}`);
    ...
    // if (!req.body.extention) return res.status(400).send();
    ...
    //Download
    const image = `data:image/${req.body.extention};base64,${req.body.image}`;
    let result;
    let gallery;
    let originalImage;
    let response;
...
router.get('/image/:id', async (req, res) => {
    ...
    if (!req.params.id) return res.status(400).send('gallery id must be provided');
...
router.get('/breakpoint/:breakpointId/image/:imageId', async (req, res) => {
    ...
    if (!req.params.breakpointId) return res.status(400).send('breakpointId must be provided');
    if (!req.params.imageId) return res.status(400).send('imageId must be provided');

    let gallery = await Gallery.findOne({ 'breakpoints._id': req.params.breakpointId, 'breakpoints.images._id': req.params.imageId, });
    if (!gallery) return res.status(404).send();

    let breakpoint = gallery.breakpoints
    ...
    let extention = breakpointFiltering.format;
    // End only for test

    let images = breakpoint.images.find(image => image._id == req.params.imageId);
    const image = Buffer.from(images.image, 'base64');

    res.writeHead(200, {
        'Content-Type': `image/${extention}`,
        'Content-Length': image.length,
        'Content-Disposition': `inline; filename=${req.params.imageId}.${extention}`,
        'Access-Control-Allow-Origin': '*',
        'Cache-Control': 'public, max-age=31557600'
    });
        ...
        try {
            Array.from(result.responsive_breakpoints).forEach((item) => {
                Array.from(item.breakpoints).forEach(async (subitem, index) => {
                    ...
                    let now = Date.now();
                    let filePath = `${path.resolve(config.get('paths.temp'))}/${now}@${subitem.width}w.${result.format}`;

                    console.log(`jpg filePath: ${filePath}`);
                    await imageService.downloadImage(subitem.url, filePath);

                    let fileAsBase64 = await fs.promises.readFile(filePath, { encoding: 'base64' });
